	...
	{foreach $animelist as $aletter => $anime}
		{foreach $anime as $a}
			<tr class="{cycle values=",alt"}">
				<td width="40%"><a align="" class="title" title="View anime" href="{$smarty.const.WWW_TOP}/anime/{$a.anidbidb}"><img class="shadow" width="35%" src="{$smarty.const.WWW_TOP}/covers/anime/{$a.anidbidb}.jpg" />  {$a.title|escape:"htmlall"}</a></td>			
				...
				<td style="text-align: center;"><a title="View at AniDB" target="_blank" href="{$site->dereferrer_link}http://anidb.net/perl-bin/animedb.pl?show=anime&aid={$a.anidbidb}">AniDB</a></td>
			</tr>
		{/foreach}
	{/foreach}
</table>
